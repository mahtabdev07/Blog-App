...
import { useBlogOperations, UpdateBlogInput } from "@/hooks/useBlogs";
...
export default function EditBlogForm({ blog }: EditBlogFormProps) {
  const { updateBlog, isUpdating, updateError } = useBlogOperations();
  ...
  const onSubmit = async (data: BlogFormData) => {
    try {
      ...
      await updateBlog(blog.id, blogData as UpdateBlogInput);
    } catch (error: unknown) {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <AnimatePresence mode="wait">
              {(errors.root || updateError) && (
                ...
                >
                  {errors.root?.message || updateError?.message || "An error occurred"}
                </motion.div>
              )}
            </AnimatePresence>
